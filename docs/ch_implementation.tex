...
In der Vorbereitung des Projektes wurde unter anderem auf die Verzweigung von \index{Open Source}Open Source-Projekten eingegangen: Es soll nicht jedesmal das Rad neu erfunden werden, eine wichtige Herausforderung im Open Source-Entwicklungsmuster ist es einerseits andere geeignete Projekte zu finden und diese dann effektiv in das eigene Projekt zu integrieren. In diesem Sinne war die erste Phase in der TeamFound-Entwicklung von \index{Recherchearbeiten}Recherchearbeiten geprägt. Für den TeamFound-Server heisst das: Es muss eine \index{Bibliothek}Bibliothek gefunden werden, welche uns die \index{Volltextindexierung}Volltextindexierung von beliebigen Dokumenten möglich macht. Diese \index{Bibliothek}Bibliothek muss dann - sofern sie dieses nicht schon selber anbietet - geeignet ans Internet gebracht werden, einerseits um neue Dokumente herunterzuladen und in den \index{Index}Index aufzunehmen, andererseits um \index{Suchanfragen}Suchanfragen entgegenzunehmen und deren \index{Ergebnisse}Ergebnisse zurückzugeben.
Diese \index{Bibliothek}Bibliothek sollte möglichst erweiterbar sein, um auch die Anforderungen von (relativ) einfachen \index{Dokumentenkategorien}Dokumentenkategorien umsetzen zu können. Für die Verwaltung der Kategorien würde eine Datenbank benötigt werden, da die Verknüpfung von Kategorien, sowie deren Details, wie Namen und Beschreibung nichts mit der Index-Bibliothek zu tun haben.

\subsection{Plattform}
\label{platform}
Die Entscheidung für eine \index{Plattform}Plattform wurde implizit durch die Entscheidung für eine \index{Bibliothek}Bibliothek zum indexieren und durchsuchen von Dokumenten getroffen. Nachdem einige Tage nach derartigen Bibliotheken Ausschau gehalten wurde, ist sehr schnell deutlich geworden, das \index{Lucene}Lucene \footnote{www.lucene.de} vom \index{Apache-Projekt}Apache-Projekt\footnote{http://www.apache.org} die am weitest entwickelte \index{Bibliothek}Bibliothek ist. Ein weiterer Vorteil ist die aktive Community rund um Lucene, so wurde während unserer Entwicklung noch ein Sprung von Version 1.5 auf 1.9 mitgemacht. Andere Bibliotheken, zum Beispiel in \index{Perl}Perl oder \index{PHP}PHP boten keine ausreichende Dokumentation oder Erweiterbarkeit.
Die ursprüngliche Lucene-Version ist in \index{Java}Java implementiert, ein weiteres Projekt bemüht sich um dessen Umsetzung in C und mittlerweile (Am Ende des Projekts hat \index{Zend Technologies}Zend Technologies\footnote{http://www.zend.com} einen \index{PHP-Wrapper}PHP-Wrapper\footnote{http://framework.zend.com/manual/en/zend.search.html} für Lucene veröffentlicht.

Somit war die Wahl der Plattform auf \index{Java}Java gefallen, eine erste mehr als Proof-of-Concept anzusehende Version von Teamfound wurde mittels zwei \index{Perl-CGI}Perl-CGI-Scripten und einem Systemaufruf der Lucene-Bibliothek umgesetzt. Dieses Vorgehen startet bei jeder Anfrage an den \index{Webserver}Webserver eine neue Java virtuel machine, was natürlich überhaupt nicht effektiv ist. 

Daher fiel eine weitere Entscheidung, für eine \index{Java-Servlet}Java-Servlet-Umsetzung, um die benötigte Umgebung für Anfragen an den Lucene-Index nicht für jedes Request neu zu schaffen. Somit besteht der Server aus einer 100\%igen Java-Umgebung und es entfällt damit die Notwendigkeit für aufwändige und zeit-kostende Wrapper um mehrere Programmiersprachen zu verbinden.
...
\index{Lucene}Lucene ist - laut eigener Aussage - der harte Teil einer \index{Suchmaschine}Suchmaschine. Diese Bibliothek erzeugt und durchsucht einen \index{Volltextindex}Volltextindex, kümmert sich aber weder darum wo die Daten herkommen, wie die Ergebnisse präsentiert werden oder wie die \index{Suchanfragen}Suchanfragen vom Benutzer an den Index kommen. Der \index{Index}Index ist hingegen bietet viele Möglichkeiten die eigenen Daten zu analysieren und in sogenannten Dokumenten in den Index zu speichern. Eigene $Analyzer$ sind mit wenig Aufwand umzusetzen (sofern die mitgelieferten nicht ausreichen), auch aufwändigere \index{Parser}Parser sind möglich, wie es zB. für \index{HTML-Seiten}HTML-Seiten notwendig ist.
Darüberhinaus kann jedes Dokument im Index beliebige $Felder$ enthalten, ein Feld ist als Inhaltselement eines Dokuments zu verstehen (Überschrift, Inhalt, Beschreibung). Dabei können völlig verschiedene Arten von Dokumenten in einem Index gespeichert werden sowie Suchanfragen direkt an ein oder mehrere Felder gestellt werden. Diese starke Erweiterbarkeit ist zum Beispiel bei der Implementierung von \index{Dokumentenkategorien}Dokumentenkategorien in TeamFound sehr wichtig gewesen (Siehe Kapitel \ref{kategorien})
...
Ein Lucene-\texttt{Document} ist die \index{Basisspeichereinheit}Basisspeichereinheit in einem von \index{Lucene}Lucene erstellten Index. Die Argumente bzw. Ergebnisse beim Einf"ugen und Suchen sind immer Lucene-\texttt{Documents}. 
...
mindestens ein \index{Inhaltsfeld}Inhaltsfeld definiert. Dieses wird in \index{Token}Token zerlegt und indiziert.
...
Der \index{Analyzer}\texttt{Analyzer} zerlegt zu \index{indizierte Felder}indizierende Felder 
in \index{Tokenstream}Tokenstreams, damit diese dann ausgewertet werden k"onnen.
Das kann beliebig komplex oder simple sein. Ein \index{Analyzer}Analyzer kann z.B. speziell auf eine 
bestimmte Sprache ausgelegt sein um Artikel, Pronomen etc. direkt ausfiltern oder einfach
anhand von $Whitespaces$ Token bilden. Als Nutzer von \index{Lucene}Lucene bestehen hier beliebig 
...
\paragraph{Query und QueryParser}


Um nun \index{indizierte Felder}indizierte Felder zu durchsuchen ist die
Grundeinheit die \index{Query}\texttt{Query}. Der Name ist etwas irref"uhrend, da es sich hierbei noch
nicht um wirkliche Anfragen handelt. Eine \texttt{Query} besteht erstmal nur aus einem 
einzelnen \index{Token}Token. Nach diesem k"onnte gesucht werden. \index{Lucene}Lucene bietet nun verschiedenste
...
bietet Lucene eine Erleichterung. Der \index{QueryParser}\texttt{QuerryParser} stellt eine einfache \index{Syntax}Syntax zur
Verf"ugung. Diese wird ausgewertet und liefert die komplexe Anfrage f"ur den \index{Index}Index.
...
\subsection{HSQLDB}
Die Entscheidung keine f"ur \index{Webapplikationen}Webapplikationen typische \index{Datenbank}Datenbank wie \index{MySQL}MySQL einzusetzen 
ist leicht erkl"art. Wie schon in Kapitel \ref{platform} beschrieben, sollte eine 
Java-Umgebung eingesetzt werden.
Das schr"ankt erstmal nur auf Datenbanken ein, die \index{JDBC}JDBC kompatibel sind.
Deshalb ist der Server auch leicht um einen weiteren \index{Datenbanklayer}Datenbanklayer zu erweitern, falls andere Datenbanken erw"unscht sind.

\index{HSQLDB}HSQLDB wurde von uns ausgew"ahlt, weil wir sie als kleine \index{Java-Bibliothek}Java-Bibliothek einbinden k"onnen
und es damit den Nutzern unseres Servers ersparen sich extra um einen \index{Datenbankserver}Datenbankserver zu k"ummern.
...
kleine Datenbank f"ur den TeamFound-Server allein.
Die "ublichen Riten wie Datenbanknutzer anlegen usw. k"onnen wir dadurch im Server selbst automatisiert regeln.
HSQLDB hat sehr gute Ergebnisse in dem \index{PolePosition benchmark}PolePosition benchmark (siehe Literaturverzeichnis PolPos) geliefert und unterst"utzt einen grossteil der \index{ANSI}ANSI-92 \index{SQL}SQL, SQL 99 und 2003 Standards bis hin zu Transaktionen.
...
Zur Erzeugung der \index{XML-Antworten}XML-Antworten (Siehe Kapitel \ref{response}) wird die \index{JDOM-Bibliothek}JDOM-Bibliothek\footnote{http://www.jdom.org} von Jason Hunter benutzt. \index{JDOM}JDOM ist eine abgewandelte \index{DOM}DOM-Implementierung\index{Document Object Model}\footnote{Document Object Model} und bietet die Möglichkeit mit einfachen Java-Code sauberes XML zu erzeugen. JDOM wird ebenfalls benutzt um die XML-Dokumente in HTML zu wandeln, sofern der Client dieses wünscht.

\section{Architektur}
\index{Architektur}
In diesem Kapitel soll ein Einblick in den Aufbau des \index{Server}Servers gegeben werden.
...
\subsection{Servlet}
\label{servlet}
Das \index{Servlet}Servlet ist der Einstiegspunkt für jegliche Anfragen an TeamFound. Dem Servlet kommt dabei die Rolle eines Wächters zu, welcher überprüft ob notwendige \index{Parameter}Parameter für die einzelnen \index{Anfragen}Anfragen vorhanden sind. Ist eine Anfrage gültig, wird diese an den sogenannten \index{Controller}Controller weitergereicht, welcher die eigentlichen TeamFound-Komponenten steuert. Dieser Controller gibt immer ein \index{Response-Objekt}Response-Objekt zurück, welches vom \index{Servlet}Servlet dann zB. als XML oder HTML serialisiert und als Antwort zum Client geschickt wird.

\subsection{Controller}
\label{controller}
Die Hauptaufgabe des \index{Controllers}Controllers besteht darin die Teilaufgaben, die sich aus einer 
...
Die Zugriffe auf den \index{Index}Index werden dabei durch eine Instanz eines \index{Indexer}Indexers erledigt.
...
Der Zugriff auf die Datenbank erfolgt "uber den \index{DBLayer}DBLayer. Dieser stellt
s"amtliche Funktionalit"at zum lesen und schreiben der \index{Datenbank}Datenbank zur
Verf"ugung.

Wenn alle Teilaufgaben erf"ullt sind (siehe Abl"aufe \ref{request}), generiert der \index{Controller}Controller aus den 
erhaltenen Informationen eine \index{Response}Response und "ubergibt diese dem \index{Servlet}Servlet.

\subsection{Indexer}
\label{indexer}
Der \index{Indexer}Indexer ist f"ur die Arbeit direkt am \index{Lucene-Index}Lucene-Index verantwortlich. Er f"ugt 
Dokumente (siehe \ref{lucene}) in den Index ein oder entfernt sie wieder. 
Ausserdem f"uhrt er Suchanfragen auf dem Index aus.
Die von uns definierte \index{Dokumentenstruktur}Dokumentenstruktur (siehe \ref{docstruktur}) ist bereits im \index{Controller}Controller bekannt, 
...
der wird haupts"achlich ben"otigt um das \index{Kategorienfeld}Kategorienfeld (siehe \ref{docstruktur}) korrekt zu zerlegen.
Ausserdem geh"ort ein einfacher \index{HTML-Parser}HTML-Parser zum Indexer, dieser soll den Text und
einige zus"atzliche Informationen (z.b. context-type) aus den HTML-Seiten extrahieren.
Weitere Parser z.b. f"ur \index{Postscript}Postscript oder \index{PDF}PDF-Dateien sind in zuk"unftigen Versionen denkbar.

Die Anfragen werden zu einem Teil durch den \index{QueryParser}QueryParser (siehe \ref{lucene}) und zum 
anderen vom \index{Indexer}Indexer generiert. Der Indexer erstellt dabei die Einschr"ankung
...
\subsection{Response}
\label{response}
Antworten werden in TeamFound immer als \index{XML}XML generiert, diese XML\-Repräsentation kann bevor sie zum Client geschickt wird, noch mittels \index{XSLT}XSLT in eine geeignete HTML\-Fassung gewandelt werden.
Eine Basisklasse \index{Response}Response übernimmt dabei das Handling des eigentlichen JDOM-Document-Objektes, während die eigentlichen Antwort\-Klassen, wie \index{SearchResponse}SearchResponse oder \index{AddPageResponse}AddPageResponse ihre eigentlichen Inhalte an die Standardelemente einer Antwort anhängen (Siehe dazu mehr in Kapitel \vref{protokoll}, Protokoll).
...
Die \index{Datenbank}Datenbank besteht zur Zeit aus 4 Tabellen (siehe Abbildung \ref{dberd}).
Die wichtigste \index{Tabelle}Tabelle des derzeitigen Standes ist die $category$-Tabelle.
Sie beinhaltet die \index{Kategorieb"aume}Kategorieb"aume der Projekte eines Servers.

Um die\index{Baum}B"aume in der \index{Datenbank}Datenbank zu repr"asentieren haben wir
\index{Verschachtelte Mengen}Verschachtelte Mengen (\index{nested sets}nested sets) eingesetzt. 
...
eine \index{rekursive Tabellenstruktur}rekursive Tabellenstruktur in der Datenbank vermeiden.
Die \index{verschachtelte Mengen}Verschachtelten Mengen boten uns eine gute Variante dies zu umgehen.
Genauere Informationen hierzu befinden sich im Kapitel \ref{katrep} .

Die \index{Tabelle}Tabelle $categoryversion$ k"onnte prinzipiell zu jeder Kategorie eine
\index{Versionsnummer}Versionsnummer mitf"uhren. Derzeit wird dies nur f"ur \index{Wurzelknoten}Wurzelknoten getan.
Damit hat jeder vom Server verwaltete \index{Kategoriebaum}Kategoriebaum eine Versionsnummer. Diese
wird benutzt um die Aktualit"at der B"aume zwischen den Clients untereinander 
...
werden in der $indexedurl$ \index{Tabelle}Tabelle gehalten. Jede Url, die \index{indiziert}indiziert wurde, wird
mit Datum in dieser Tabelle gespeichert. Somit kann schnell festgestellt werden,
ob eine \index{URL}URL schon einmal hinzugef"ugt wurde und zu welchen Kategorien sie bisher
zugeordnet war.
Da der \index{Index}Index selbst auch doppelte Eintr"age zul"asst, 
...
auch in verschiedenen \index{Kategorien}Kategorien in ein und demselben \index{Baum}Baum zulassen m"ochten.
...
Im \index{Index}Index wird allerdings zu jeder \index{URL}URL nur ein einziges Dokument (siehe \ref{luckomponenten}) abgelegt.
...
Das passiert in der \index{Datenbank}Datenbank als auch im Dokument des Indexes.
Dadurch werden bei einer \index{Suchanfrage}Suchanfrage innerhalb einer Kategorie automatisch 
auch alle Urls geliefert, die \index{Kindkategorie}Kindkategorien dieser Kategorie zugeordnet wurden.
...
Die \index{Bibliothek}Bibliothek \index{Lucene}Lucene macht bestimmte Vorgaben wie die Textdokumente, die von Lucene
\index{indiziert}indiziert werden sollen, zu erstellen sind. (siehe \ref{luckomponenten})
...
Jede Html-Seite, die dem \index{Index}Index hinzugef"ugt werden soll, muss erstmal serverseitig heruntergeladen werden.
...
ein \index{Lucene-Dokuments}Lucene-Dokument verpackt.

Das erste Feld (siehe \ref{luckomponenten}), welches ein Teamfounddokument 
enth"alt, ist
ein \index{Schl"usselwortfeld}Schl"usselwortfeld. In diesem wird die Url zu der dieses Dokument geh"ort
...
Sie dient ausschlie"slich als n"ahere Beschreibung eines \index{Suchergebniss}Suchergebnisses einer Anfrage.
...
Die einfachste L"osung war die ID einer \index{Kategorie}Kategorie aus der Datenbank als \index{Token}Token 
...
so dass er die \index{Token}Tokens dieses Feldes einfach anhand des Trenncharakters generiert.
Auf diese Weise k"onnen wir die \index{Anfragen}Anfragen leicht so erweitern, dass sie mithilfe von \index{Kategorien}Kategorien eingeschr"ankt werden k"onnen.
...
Da ein wichtiger Teil des derzeitigen Servers die Verwaltung der \index{Kategorien}Kategorien 
...
\index{baumstrukturiert}baumstrukturiert und werden in Form von \index{verschachtelten Mengen}verschachtelten Mengen in der
\index{Datenbank}Datenbank abgelegt.
...
Anhand dieser Werte kann die \index{Mengen-Baumstruktur}Mengen-Baumstruktur in der Datenbank nachgebaut und 
der Baum durchlaufen werden.
Es ist leicht zu erkennen, dass der rechte Wert eines Knotens gr"o"ser ist als
alle Werte der \index{Kindknoten}Kindknoten und genauso ist der linke Wert kleiner.
...
Auf der anderen Seite muss bei sehr vielen Anfragen der \index{Baum}Baum durchsucht werden.
Allein das Anlegen einer neuen \index{Kategorie}Kategorie hat zur Folge, das jeder Client den
neuen \index{Kategorienbaum}Kategorienbaum vom Server erfragt.
...
eines Baumes in einem \index{Index}Index abzulegen.
...
Das erkl"art sich darin, das alle Dokumente, die einer \index{Unterkategorie}Unterkategorie 
...
\label{request}
Wie Aufrufe vom \index{Client}Client im \index{Server}Server abgearbeitet werden, soll kurz in diesem Abschnitt 
behandelt werden.
...
Auftrag angestossen.
Bei nicht korekten Aufrufen wird eine \index{ErrorResponse}\texttt{ErrorResponse} erstellt.

Der \index{Controller}Controller erstellt nun eine Verbindung zur \index{Datenbank}Datenbank 
und einen Indexer (siehe Kapitel \ref{indexer}).
Als n"achstes "uberpr"uft er anhand der \index{Datenbank}Datenbank ob die Url schon existiert.
...
Danach wird ein Dokument erstellt und durch den \index{Indexer}Indexer dem \index{Index}Index hinzugef"ugt.
(siehe hierzu Kapitel \ref{docstruktur})
Als letztes muss noch die \index{Datenbank}Datenbank angeglichen werden. (siehe Kapitel \ref{datenbank})
...
aus dem Index gel"oscht werden. Dabei wird eine Kopie des Dokuments an den \index{Controller}Controller
...
Assoziationen zwischen Url und \index{Kategorien}Kategorien werden in der \index{Datenbank}Datenbank gesetzt.
...
den Kategorien nichts. Auch diese M"oglichkeit wird anahnd der \index{Datenbank}Datenbank "uberpr"uft.
Sollte der Fall eintreten passiert gar nichts und die Erfolgsmeldung wird 
sofort an den \index{Client}Client gesendet. Es ist einfach vorstellbar, dass ein Benutzer ausversehen
...
Der am h"aufigsten ben"otigte Dienst des Servers ist die \index{Suche nach Dokumenten}Suche nach Dokumenten.
...
Funktion des \index{Controllers}Controllers (siehe \ref{controller}) auf.

Eine "Ubersicht zum Ablauf einer \index{Suchanfrage}Suchanfrage biete die Abbildung \ref{search}.
Die Suchanfrage enth"alt zuerst einen vom User eingegebenen Text, der in der
\index{Lucene}Lucene $QuerySyntax$ formuliert ist. 
(siehe \texttt{QueryParser} in Kapitel \ref{luckomponenten})
Zus"atzlich wird ein Array von \index{Kategorie-ID}Kategorie-IDs "ubergeben. Die \index{Kategorien}Kategorien wurden vom
Nutzer am Client ausgew"ahlt. Da alle Clients immer ihre \index{Kategorieb"aume}Kategorieb"aume 
...
\index{Indexer}Indexer (siehe Kapitel \ref{indexer}) erstellt. Dieser baut anhand der \index{Parameter}Parameter
eine $Query$ (Kapitel \ref{luckomponenten}), die dann durch \index{Lucene}Lucene bearbeitet wird.
...
durch den \index{Controller}Controller ausgewertet und eine $Response$ wird erstellt. 
Diese kann dann die \index{HTML-Antwort}HTML- oder \index{XML-Antwort}XML-Antwort generieren, welche an den Client gesendet wird.


\subsection{GetCategoriesRequest}
\label{getcatrequest}
\index{GetCategoriesRequest}
\begin{figure}
...
In den vorangegangenen Kapiteln wurden \index{Kategorien}Kategorien schon ziemlich ausgiebig erl"autert.
...
F"ur diese Aufgabe muss der \index{Controller}Controller nur zwei \index{Datenbankabfragen}Datenbankabfragen machen und kann 
dann die entsprechende Response f"ullen.
Als erstes werden daf"ur die vollst"andigen Informationen "uber die \index{Kategorie}Kategorie, deren
...
\end{verbatim}
ergibt bei dem \index{Kategoriebaum}Kategoriebaum aus Abbildung \ref{katstruktur} folgende Antwort:
...
\section{M"ogliche Verbesserungen im derzeitigen Server}
\begin{enumerate}
\item Update indizierter Seiten ohne den Index lahmzulegen.

	Da lesender und schreibender Zugriff exklusiv voneinander getrennt ablaufen, kann ein 
	Update zu einer langen Blockierung des Servers f"uhren.
	Es gibt eine M"oglichkeit dies zu umgehen. Lucene kann verschiedene Segmente anlegen
	wenn der Index vergr"ossert wird. Es ist m"oglich den lesenden Zugriff auf 
	ein ''altes'' Segment zu gew"ahren, w"ahrend ein neues Segment mit den aktualisierten
	Dokumenten angelegt wird. Der lesende Zugriff muss auf diese Weise nur blockiert werden,
	wenn die Segmente wieder Zusammengef"uhrt werden.
\item Indizierung anhand der Sprache.
Man k"onnte bei Webseiten den Analyzer anhand der 
	gelieferten Sprachinformation aus dem HTTP-Header w"ahlen und somit verhindern 
	unn"utze Worte wie z.B. Artikel zu indizieren.
\item Bessere Implementierung der Download Instanz.
Wir nutzen zur Zeit eine eigene Klasse, 
		die angeforderte Adressen herunterl"ad. Deshalb gibt es z.B. Probleme mit Frames.
		Die Implementierung muss verbessert werden oder es muss eine geeignete Bibliothek 
		gefunden werden.
\item Finden einer geeigneten L"osung f"ur gleiche Seiten mit unterschiedlicher URL.

	Man betrachte zum Beispiel die Adressen \texttt{http://xzy.de} und\\ \texttt{http://xzy.de/index.html}. Diese sind 
	wahrscheinlich genau dieselbe Seite. Sollten solche Adressen an den Server "ubergeben
	werden, w"urde er diese als zwei verschiedene Seiten behandeln. Auf diese 
	Weise k"onnen redundante Dokumente entstehen. 
	
	Ein Ansatz dies zu verhindern w"are es
	Hashwerte zu indizierten Seiten zu erstellen und zu speichern. Anhand von vergleichen der
	Hashwerte bei gleichem URL-Stamm k"onnte man das Problem umgehen. 
\end{enumerate}

\section{Probleme}
W"ahrend der Entwicklung des Servers sind keine grossen Probleme entstanden. 
Dennoch k"onnen vielleicht ein paar kleinere genannt werden.

\begin{itemize}
\item Versionskonflikte beim Java Environment
	
	W"ahrend der Entwicklung stellten wir unerwartet fest, dass einer von uns mit J2SE 1.4.2
	und der andere mit J2SE 5.0 arbeitet. Daraufhin diskutierten wir kurz mit allen
	ob es sinnvoll w"are Kompatibilit"at zu 1.4.2 zu erhalten. Die Entscheidung 
	viel schnell darauf auf 5.0 Basis weiterzumachen. Erstens h"atte sonst Code umge"andert
	werden m"ussen und zweitens einigten wir uns das es Release 5.0 inzwischen schon 
	lange genug gibt.
\item Milestone 2

	Milestone 2 brachte eine v"ollige Umstellung auf Java und Tomcat. Hinzu kam die
   	Erweiterung um eine Datenbank und tiefere Einarbeitung in Lucene. 
	Es waren also erstens viele neue
	Techniken zu lernen und zweitens noch eine ganze Menge Probleme zu l"osen.
	Hinzu kamen Pr"ufungen und vorlesungsfreie Zeit. Dadurch kam es zu einem
	kleinerem Motivationstief, was erst mit n"aherr"ucken des Vortragtermins "uberwunden 
	wurde.
\end{itemize}
...
F"ur unser \index{Protokoll}Protokoll kamen prinzipiell viele verschiedene M"oglichkeiten in Betracht. 

Die erste, und einfachste Designentscheidung war, ein Protokoll zu benutzen, welches auf \index{TCP/IP}TCP/IP aufsetzt. Somit k"onnen schonmal alle Computer die "uber einen Internet-Zugang verf"ugen prinzipiell mit unseren Componenten kommunizieren.

Der zweite Schritt war die Festlegung unseres Protokolls nicht direkt auf \index{TCP/IP}TCP/IP aufzusetzen, sondern erst auf dem \index{HTTP}HTTP Protokoll. Diese Entscheidung viel auch sehr schnell und war f"ur uns naheliegend, da wir von Anfang an vorhatten \index{Browser-Plugins}Browser-Plugins zu programmieren, und jeder \index{Internet-Browser}Internet-Browser HTTP ebenfalls unterst"utzt. 

Als weitere Grundlage f"ur unsere \index{Designentscheidungen}Designentscheidungen war unser Wunsch, die M"oglichkeit einen ganz einfachen \index{Web-Client}Web-Client bereitstellen zu k"onnen, so da"s TeamFound auch ohne Installation einer Browser-Erweiterung ausprobiert und genutzt werden kann. Diese Entscheidung f"uhrte dann auch direkt zu der eigentlichen Basis unseres Protokolls:
\index{HTML-Antwort}
\index{HTTP-GET Variablen}
\index{XML-Antwor}
\begin{description}
...
Die \index{Implementierung}Implementierung des \index{Protokolls}Protokolls, sowie auch alle anderen Komponenten von TeamFound, hatte wir in kleine Schritte unterteilt, unsere Milestones. So hat das Protokoll f"ur Milestone 1 z.B. noch keine Unterst"utzung f"ur XML-Antworten und Kategorien gehabt.

Auf Basis dieser gundlegenden Entscheidungen, haben wir also begonnen sowohl die \index{HTTP-GET}HTTP-GET Anfragen zu spezifizieren als auch (ab Milestone 2) \index{XML-Tags}XML-Tags zu definieren, um die volle Funktionalit"at unserer Implementation nutzen zu k"onnen. 

\section{Interface Milestone 1}
\index{Interface Milestone 1}
...
\index{Interface Milestone 2}
Alle Anfrage-Parameter werden in Form von \index{HTTP GET}HTTP \index{GET}GET oder \index{POST}\index{HTTP POST}POST Variablen übertragen. Prinzipiell gibt es eine Unterscheidung, ob f"ur jedes Kommando eine eigene URL verwendet wird, oder ob das Kommando in Form einer weiteren HTTP-GET Variablen mit "ubertragen wird. Unsere Implementation des  Servers verwendet den zus"atzlichen HTTP-GET Parameter \texttt{command}. In diesem \index{Interface}Interface haben wir aber beide M"oglichkeiten spezifiziert.
...
Die \index{Return-Codes}Return-Codes stehen immer in den Tags $<$return-value$>$xx$<$/return-value$>$. Die Description ist optional.
...
Mit diesem einfachen \index{Link}Link, kann das l"astige Kopieren umgangen werden, indem "uber \index{Javascript}Javascript die URL der aktuell im eigenen \index{Browser}Browser angezeigten Seite, durch einen einfachen Click auf diesen \index{Bookmark}Bookmark, direkt an den TeamFound-Server gesendet wird. (Beispiel ist f"ur Milestone 1)

\subsection{Implementation}
Die \textit{extrem einfache} Implementation des Web-Clients f"ur eine \index{Suchanfrage}Suchanfrage entsprechend Milestone 2 sieht wie folgt aus:
\begin{verbatim}
...
\end{verbatim}
Bei dem Design der \index{TeamFound-Interfaces}TeamFound-Interfaces war diese M"oglichkeit der \index{HTML-Formulare}HTML-Formulare die entscheidende Grundlage. Alle Anfragen an den Server k"onnen mit HTML-Formularen ohne Zus"atze (wie \index{Javascript}Javascript, \index{Applets}Applets, \index{Extension}Extensions, etc.) generiert werden.

\section{Firefox-Toolbar}
\index{Firefox Toolbar}
Der \index{Mozilla Firefox}Mozilla Firefox l"asst sich durch sogenannte \textit{Extensions} sehr einfach erweitern. Als grundlegende Techniken werden daf"ur \index{XUL}XUL (\index{XML User Interfaces Language}XML User Interface Language) und \index{Javascript}Javascript ben"otigt. Desweiteren gibt es eine exakt vordefinierte Verzeichnisstruktur sowie eine Beschreibungs-Datei (\texttt{install.rdf}) im \index{RDF}RDF (\index{Resource Description Framework}Resource Description Framework) Format, umd die Erweiterung automatisiert installierbar in Firefox-Browsern zu machen.
...
\subsection{Dateien}
Die \index{Firefox Toolbar!Verzeichnisstruktur}Verzeichnisstruktur der TeamFound Firefox Extension sowie alle enthaltenen Dateien sind:
\begin{verbatim}
chrome.manifest
install.rdf
...
\index{Firefox Toolbar!overlay.xul}
\index{Firefox Toolbar!overlay.js}
\index{Firefox Toolbar!settings.xul}
\index{Firefox Toolbar!settings.js}
\begin{description}
\item[overlay.xul] Definiert das Aussehen der \index{Toolbar}Toolbar
\item[overlay.js] Definiert die Funktionalit"at der Toolbar
\item[settings.xul] Definiert das Aussehen des \index{Einstellungen-Dialog}Einstellungen-Dialogs
...
Das originale \index{Changelog}Changelog in englischer Sprache der Firefox-Toolbar befindet sich im Anhang \vref{changelog}.
\\\\
Die wichtigsten Funktionen der \index{Toolbar}Toolbar sind:
\begin{description}
\index{Firefox Toolbar!onLoad}
...
\index{Firefox Toolbar!loadCategories}
...
\index{Firefox Toolbar!onLoadCategoriesFinished}
...
\index{Firefox Toolbar!onSettings}
...
\index{Firefox Toolbar!onSearch}
...
\index{Firefox Toolbar!myExternSearch}
...
\index{Firefox Toolbar!onAddPage}
...
\index{Firefox Toolbar!addCategory}
...
\index{Firefox Toolbar!onTeamFoundSearchFinished}
...
\index{Internet Explorer Toolbar}
Auch beim \index{Internet Explorer}Internet Explorer stand zunächst die Frage wie dieser überhaupt zu erweitern ist. 
...
war \index{CodeProject}CodeProject \footnote{www.codeproject.com}. 
Mit 2,8 Mio Mitgliedern ist sie die größte Community Seite für die Windows Entwicklung. 
Eine Suche nach ``IE Toolbar'' führte zu einer \index{.NET}.NET basierten Bibliothek 
für die Entwicklung von \index{BandObjects}BandObjects, also Band Objekten 
\footnote{http://www.codeproject.com/csharp/dotnetbandobjects.asp}.
...
Ein zentrales Konzept zur Erweiterung des \index{Internet Explorer}Internet Explorers, sind \index{Explorer Bars}Explorer Bars. 
...
Die \index{Werkzeugleiste}Werkzeugleiste des Internet Explorers ist eine sogenannte ``Rebar'', 
also ein \index{Container}Container der wiederum mehrere Werkzeugleisten enthalten kann. 
Diese Werkzeugleisten werden Toolbands genannt. Erstmalig wurde das Konzept 
mit dem \index{Internet Explorer}Internet Explorer 5.0 eingeführt um die Radio Leiste zu realisieren. 
...
Sie kann ausser im Explorer und \index{Internet Explorer}Internet Explorer auch in der \index{Taskleiste}Taskleiste erscheinen.

\subsection{Entwicklung der Band Objekte}

Die Entwicklung dieser Komponenten basiert, wie ein sehr großer Teil der 
Windowsentwicklung auf \index{COM}COM. Das \index{Component Object Model}Component Object Model von \index{Microsoft}Microsoft 
ist eines der ältesten Komponentensysteme und bildet zusammen mit dem 
\index{Windows API}Windows API die Grundlage für die Entwicklung von Windowssoftware.
Da die Toolbar aus solchen Bandobjekten besteht, müssen also COM Objekte geschrieben werden.
 
\subsection{COM, Schnittstellen, Klassen, Registry}

Im folgende soll ein kurzer Überblick über \index{COM}COM gegeben werden, 
der die Schritte auf dem Weg zu Entwicklung der \index{Band Object}Band Objects erklärt.

\paragraph{COM Schnittstellen}
\index{COM Schnittstellen}
Zentrales Konzept von \index{COM}COM ist das Interface. In der COM Terminologie
...
Namen ``OLE Automatisierung'' hinzu, welches ähnliche Konzepte wie Java Reflection bietet und somit den Scriptsprachen den Zugang zu COM ebenete.
Jede Schnittstelle wird eindeutig über einen GUID - Global Unique Identifier - indentifiziert.
  
\paragraph{COM Komponenten}

Eine Komponente in der \index{COM}COM Terminologie ist eine abgeschlosse binäre Einheit, 
welche Funktionalität über eine Schnittstelle bereitstellt. 
Komponenten sind also in Programmen (Executables) oder Bibliotheken (\index{Dynamic Link Library}Dynamic Link Libraries) enthalten.

\paragraph{Registry}
\index{Registry}
Normalerweise, auch wenn das problemlos möglich wäre, 
wird eine \index{COM}COM Bibliothek nicht direkt referenziert und geladen. 
...
Alle \index{Band Objekte}Band Objekte sind wiederum \index{COM}COM Komponenten, welche bestimmte Schnittstellen 
implementieren müssen. Diese Schnittstellen werden vom \index{Internet Explorer}Internet Explorer erwartet und abgefragt. 
Für die Entwicklung von \index{Band Objekte}Band Objekten müssen in jedem Fall die folgenden Schnittstellen implementiert werden:

\paragraph{IUnknown}
\index{IE Toolbar!IUnknown}
Das ist die Basisschnittstelle welche jedes \index{COM}COM Objekt implementieren muss. 
Sie bietet Funktionen zum Abrufen weiterer Schnittstellen sowie zur \index{Referenzzählung}Referenzzählung.

\paragraph{IClassFactory}
\index{IE Toolbar!IClassFactory}
Bietet Funktionen zum Erzeugen neuer Instanzen von COM Klassen.

\paragraph{IDeskBand}
\index{IE Toolbar!IDeskBank}
...
\index{IE Toolbar!IObjectWithSite}
...
\index{IE Toolbar!IPersistStream}
...
Alles was im Abschnitt \index{COM}COM und \index{Band Object}Band Object Entwicklung beschrieben worden ist, 
nimmt einem die Band Objekt Library ab. Der ganze ``schmuzige'' COM Teil wird durch 
eine \index{CSharp Klasse}CSharp Klasse gekapselt welche von \index{UserControl}UserControl erbt und all die Schnittstellen implementiert. 
...
Alle anderen Bibliotheken werden ebenfalls im globalen \index{Assemblycache}Assemblycache registriert.
...
\paragraph{Dateien}
Folgende Dateien sind im Projekt enthalten:
...
Es gibt zwei \index{Views}Views. Einen für die Eingabe der Suchdaten, dies ist die eigentliche Toolbar, und einen für die Darstellung der Ergebnisse. 

\begin{figure}
\begin{center}
\epsfig{file=bilder/ietoolbar, width=12cm}
\end{center}
\caption{Internet Explorer Toolbar (v0.2)}
\label{ietoolbar}
\end{figure}

Die Buttons auf der Toolbar rufen vier verschiedene Funktionen im \index{Controller}Controller auf.

\paragraph{Controller}

Der \index{Controller}Controller enthält die Logik des Clients bzw. delegiert die Aufrufe an den Server, welcher wiederum Logik enthält.
Es gibt vier wesentliche Funktion im Controller. Man könnte auch von Anwendungsfällen reden.

\begin{description}
\item[Search] Ruft den Server mit den übergebenen Suchworten und den Kategorien auf. Sobald das Suchergebniss verfügbar ist, wird ein Ereignis ausgelöst. Der View für die Suchergebnisse, reagiert darauf und stellt diese dar.
\item[AddPage] Diese Funktion zeigt ein Fenster zur Auswahl einer Kategorie und zur eingabe einer URL an. Wird dieses Fenster erfolgreich geschlossen, also mit OK statt mit Abbrechen oder dem X, wird das addpage Kommando auf dem Server aufgerufen und so die Seite dem Index hinzugefügt.
\item[AddCategorie] Ruft das addcategorie Kommando auf dem Server auf, und legt damit eine neue Kategorie an.
\item[EditSettings] Zeit ein Fenster zur Bearbeitung der Server URL an. Bei Erfolg, wird das Ergebniss in der Registry gespeichert.
\end{description}

\paragraph{Modell}

Das \index{Modell}Modell besteht aus der Klasse \index{Category}Category. Ein Category Objekt kann selber wieder Kategorie Objekte enthalten. Somit können die Kategorien vom Server abgeholt und im Speicher gehalten werden. Die Views können über die Eigenschaft Categories, des Controllers auf die Kategorien zugreifen und Anzeigen. Der Controller wiederum aktualisiert die Kategorien wenn es erforderlich ist.


\subsection{Fazit Internetexplorer Erweiterung}
...
Bei der Internet Explorer Variante liegen die Vorteile in der Flexibilität. Da ich an keine Sprache gebunden bin um die Erweiterung zu entwickeln, kann ich auch beliebe Features realisieren. Auch habe ich Zugriff auf die gesamte Umgebung, innerhalb und ausserhalb der Browsers. Somit ergeben sich natürlich noch viel mehr Möglichkeiten.
