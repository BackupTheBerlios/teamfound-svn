...
\subsection{Kernbibliothek Apache Lucene}
\label{lucene}
...
Es ist sinnvoll weitere Felder zu definieren, die nicht indiziert oder zerlegt,
sondern als zus"atzliche Information gespeichert werden. Ein eideutiges Schl"usselwort
...
\paragraph{Analyzer}
	...
	Der \texttt{Analyzer} ist die Komponente, welche zu indizierende Felder 
in Tokenstreams umwandelt, damit diese dann ausgewertet werden k"onnen.
Das kann beliebig komplex oder simple sein. Ein Analyzer kann z.B. speziell auf eine 
bestimmte Sprache ausgelegt sein und Artikel Pronomen etc. direkt ausfiltern oder einfach
anhand von $Whitespaces$ Token bilden. Als Nutzer von Lucene sind hier beliebig 
M"oglichkeiten selbst zu Erweiterungen um eigene Anspr"uche zu erf"ullen.  
...
HSQLDB hat sehr gut Ergebnisse in dem PolePosition benchmark (siehe Literaturverzeichnis PolPos) geliefert und unterst"utzt einen grossteil der ANSI-92 SQL,SQL 99 und 2003 Standards bis hin zu Transaktionen.
...
Ausserdem sorgt er f"ur den Download der zu indezierenden Dokumente.
...
Wenn alle Teilaufgaben erf"ullt sind generiert der Controller aus den 
...
Die von uns definierten Dokumente (siehe \ref{}) werden im Indexer aus den 
gegebenen Informationen erstellt. 
Zur Indizierung benutzt der Indexer einen eigenen Analyzer (siehe \ref{lucene}), 
der haupts"achlich ben"otigt wird um das Kategorienfeld korekt zu zerlegen.
